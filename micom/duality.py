...
def fast_dual(model, prefix="dual_"):
    ...
    for constraint in model.constraints:
        ...
        if constraint.lb == constraint.ub:
            const_var = prob.Variable(
                prefix + constraint.name + "_constraint",
                lb=None, ub=None)
            ...
            for variable, coef in coefs.items():
                coefficients.setdefault(
                    variable.name, {})[const_var.name] = sign * coef
        else:
            if constraint.lb is not None:
                lb_var = prob.Variable(
                    prefix + constraint.name + "_constraint_lb",
                    lb=0, ub=None)
            ...
            if constraint.ub is not None:
                ub_var = prob.Variable(
                    prefix + constraint.name + "_constraint_ub",
                    lb=0, ub=None)
            ...
            for variable, coef in coefficients_dict.items():
                if constraint.lb is not None:
                    coefficients.setdefault(
                        variable.name, {})[lb_var.name] = -sign * coef
                if constraint.ub is not None:
                    coefficients.setdefault(
                        variable.name, {})[ub_var.name] = sign * coef
    ...
    for variable in model.objective.variables:
        ...
        coefs = {model.variables[vid]: coef for vid, coef in
                 coefficients[variable.name].items()}
