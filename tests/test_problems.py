...
class TestOptcom():
    ...
    def test_is_consistent(self, community, strategy):
        ...
        assert np.allclose(sol.community_growth, 0.873922)
        assert np.allclose(sol.growth_rates, 0.873922)
    ...
    def test_must_share(self, community, strategy):
        ...
        assert sol.community_growth > 0.4
        assert (sol.growth_rates > 0.4).any()
