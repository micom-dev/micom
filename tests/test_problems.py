...
class TestLinearOptcom():
    ...
    def test_good_mingrowth(self, community, f):
        cobj, gcs = community.optcom(strategy="linear", min_growth=f)
        assert np.allclose(cobj, 0.873922)
        assert np.allclose(gcs, 0.873922)
    ...
    def test_must_share(self, community, strategy):
        ...
        cobj, gcs = community.optcom(strategy=strategy, min_growth=1.0)
        assert cobj is None
        assert np.isnan(gcs).all()
        cobj, gcs = community.optcom(strategy=strategy, min_growth=0.0)
        assert cobj > 0.4
        assert (gcs > 0.4).any()
